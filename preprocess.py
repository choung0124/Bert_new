...
def preprocess_data(json_data, tokenizer, label_to_id, relation_to_id):
    ...
    for entity in sorted(json_data["entities"], key=lambda x: x["span"]["begin"]):
        ...
        try:
            entity_start_idx = next(i for i, (start, end) in enumerate(sentence_token_offsets) if abs(start - (entity["span"]["begin"] - boundary)) <= 3)
        ...
        try:
            entity_end_idx = next(i for i, (start, end) in enumerate(sentence_token_offsets) if abs(end - (entity["span"]["end"] - boundary)) <= 3)
        except StopIteration:
